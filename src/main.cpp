...
struct Ball
{
    ...
    void update()
    {
        ...
        // float dt = Clamp(dt, 1.0f / 120.0f, 1.0f / 60.0f);
        float dt = GetFrameTime();

        speedTimer += dt;

        if (speedTimer >= 1.0f) {   // once per second
            speedBall();
            speedTimer = 0.0f;
        }
    ...
    bool checkPaddleCollision(Vector2 paddleStart, Vector2 paddleEnd, float paddleThickness)
    {
        ...
        if (!CheckCollisionCircleLine({x, y}, radius , paddleStart, paddleEnd))
...
{
protected:
    float paddle_pos_start = GetScreenHeight() / 4;
    float paddle_pos_end = GetScreenHeight() / 2;
    float maximum_boundaries = GetScreenHeight();
    int score = 0;
    int speed = 400;
...
public:
    ...
    void move_paddle(char c)
    {
        ...
        {
            paddle_pos_start = max((float)0, paddle_pos_start - speed * dt);
            paddle_pos_end -= speed * dt;
        }
        else if (c == 'd' && paddle_pos_end + 1 <= maximum_boundaries)
        {
            paddle_pos_end = min(maximum_boundaries, paddle_pos_end + speed * dt);
            paddle_pos_start += speed * dt;
        }
    }
...
public:
    ...
    GameScreen()
    {
        // ENABLE SMOOTHING FLAGS BEFORE InitWindow
...
private:
    ...
    // {
    //     screen.ball.x += screen.ball.speedx * dt;
    //     screen.ball.y += screen.ball.speedy * dt;
    // }
...
public:
    void run_game_loop()
    {
        ...
        while (!WindowShouldClose())
        {
            ...
            // {
            //     UpdatePhysics(FIXED_DT); // movement, collisions
            ...
            float ballSpeed = sqrt(screen.ball.speedx * screen.ball.speedx + screen.ball.speedy * screen.ball.speedy);
            EndDrawing();
        }
    }
};
